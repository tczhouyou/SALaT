...
class ObsSet2D:
    ...
    def add_twoRec_obs(self, pos, r, width, length):
        wx = np.cos(np.pi/2 + r) * width
        wy = np.sin(np.pi/2 + r) * width
        lx = np.cos(r) * length
        ly = np.sin(r) * length
        ...
        self.poses.append(pos)
        self.add_polygon_obs([p1,p6,p7,p8, p1])
        self.add_polygon_obs([p2,p3,p4,p5, p2])
    ...
    def plot(self, ax):
        ...
        for i in range(np.shape(self.poses)[0]):
    ...
    def get_envs(self, queries):
        if self.exp_name == 'goAroundObsR10':
            ...
            for i in range(np.shape(queries)[0]):
                ...
                # env.add_openRec_obs(query[0:2], query[2], 2, 5)
                # env.add_openRec_obs(query[3:5], query[5], 2, 5)

                env.add_openRec_obs(query[0:2], query[2], 3, 4)
                env.add_openRec_obs(query[3:5], query[5], 3, 4)
                envs.append(env)

            return envs
        elif self.exp_name == 'OriLine':
            envs = []
            for i in range(np.shape(queries)[0]):
                env = ObsSet2D()
                query = queries[i,:]
                env.add_openRec_obs([0,0], query[0], 3, 4)
                envs.append(env)

            return envs
        elif self.exp_name == 'DockingWithObs':
            envs = []
            for i in range(np.shape(queries)[0]):
                env = ObsSet2D()
                query = queries[i, :]
                ...
                env.add_openRec_obs(query[0:2], query[2], 3, 4)
                env.add_openRec_obs(query[3:5], query[5], 3, 4)
                env.add_circle_obs(origin=[query[6], query[7]], radius=4.5)
                envs.append(env)

            return envs
        elif self.exp_name == 'OriObs':
            envs = []
            for i in range(np.shape(queries)[0]):
                env = ObsSet2D()
                query = queries[i, :]

                env.add_circle_obs(origin=[query[0], query[1]], radius=3)
                envs.append(env)

            return envs
        elif self.exp_name == 'DockingWithObsReturn':
            envs = []
            for i in range(np.shape(queries)[0]):
                env = ObsSet2D()
                query = queries[i, :]
                ...
                env.add_openRec_obs(query[0:2], query[2], 3, 4)
                env.add_twoRec_obs(query[3:5], query[5], 3, 4)
                env.add_circle_obs(origin=[query[6], query[7]], radius=4.5)
                envs.append(env)
...
def checkDocking(trajs, tequeries, samples, orig_samples, exp_name="Docking"):
    orig_t = np.linspace(0, 1, orig_samples)
    curr_t = np.linspace(0, 1, samples)
    retraj = np.zeros(shape=(np.shape(trajs)[0], samples, np.shape(trajs)[-1]))
    for i in range(np.shape(trajs)[0]):
        tetraj = trajs[i, :, :]
        for j in range(np.shape(tetraj)[-1]):
            retraj[i, :, j] = np.interp(curr_t, orig_t, tetraj[:, j])

    obsExp = ObsExp(exp_name=exp_name)
    envs = obsExp.get_envs(tequeries)
    failure = 0
    success_sets = []
    for i in range(np.shape(retraj)[0]):
        env = envs[i]
        traj = retraj[i, :, :]
        start = tequeries[i,[0,1]]
        goal = tequeries[i,[3,4]]
        if np.linalg.norm(traj[0,:] - start) > 2 or np.linalg.norm(traj[-1,:] - goal) > 2:
            failure = failure + 1
            continue

        iscollision = False
        for j in range(np.shape(traj)[0]):
            if env.isCollision([traj[j,0], traj[j,1]]):
                failure = failure + 1
                iscollision = True
                break

        if not iscollision:
            success_sets.append(i)

    return failure, success_sets



def checkDockingReturn(trajs, tequeries, samples, orig_samples, exp_name="DockingReturn"):
    orig_t = np.linspace(0, 1, orig_samples)
    curr_t = np.linspace(0, 1, samples)
    retraj = np.zeros(shape=(np.shape(trajs)[0], samples, np.shape(trajs)[-1]))
    for i in range(np.shape(trajs)[0]):
    ...
    for i in range(np.shape(retraj)[0]):
        ...
        traj = retraj[i, :, :]
        start = tequeries[i,[0,1]]
        if np.linalg.norm(traj[0,:] - start) > 2 or np.linalg.norm(traj[-1,:] - start) > 2:
            failure = failure + 1
            continue
